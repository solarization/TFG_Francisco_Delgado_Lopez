...
- =fProd=: simula el comportamiento de un sistema fotovoltaico bajo diferentes condiciones de irradiancia y temperatura. Tiene los siguientes argumentos:
  - =inclin=: puede ser un objeto de clase =Gef= o un =data.frame= (o =data.table=). En el segundo caso deberá contener columnas llamadas =Gef= y =Ta=.
  - =module=: lista de valores numéricos con la información sobre el módulo fovoltaico.
  - =generator=: lista de valores numéricos con la información sobre el generador.
  - =inverter=: lista de valores numéricos con la información sobre el inversor DC/AC.
  - =effSys=: lista de valores numéricos con la información sobre las pérdidas del sistema.
    #+begin_src R
...
La función =prodGCPV= toma el resultado de =fProd= para obtener un objeto de clase =ProdGCPV=.
#+begin_src R
