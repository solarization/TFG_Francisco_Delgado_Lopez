...
En la figura \ref{fig:org453e1b3}, se muestra un diagrama que resume los pasos que se siguen a la hora de calcular la producción de sistemas fotovoltaicos.
\begin{figure}[]
\centering
...
\caption{\label{fig:org453e1b3}Procedimiento de cálculo}
\end{figure}
Estos pasos son:
\begin{enumerate}
...
Es importante resaltar que estos perfiles proceden de medias sobre largos períodos, y de ahí que, como es observable en la figura \ref{fig:org3611446}, las fluctuaciones propias del movimiento de nubes a lo largo del día queden atenuadas y se obtenga una curva sin alteraciones.
\begin{figure}[]
\centering
\includegraphics[keepaspectratio,width=0.8\textwidth,height=0.5\textheight]{figuras/RgRd.pdf}
\caption{\label{fig:org3611446}Perfil de irradiancia difusa y global obtenido a partir del generador empírico de \cite{Collares-Pereira.Rabl1979} para valores de irradiancia tomadas cada 10 minutos}
\end{figure}

\subsection{Transformación al plano del generador}
\label{sec:org4f3d6ee}
...
\end{itemize}
\item \textbf{Irradiancia de albedo} \(R(\beta ,\alpha)\): Se considera isotrópica debido a su baja contribución a la radiación global. Se calcula a partir de la irradiancia global en el plano horizontal usando un coeficiente de reflexión, \(\rho\)\nomenclature[rho]{\(\rho\)}{Coeficiente de reflexión del terreno para la irradiancia de albedo}, que depende del terreno. En la ecuación \ref{eq:albedo-plano-generador}, se utiliza el factor \(\frac{1-cos(\beta)}{2}\), complemetario al factor de visión de la difusa isotrópica (figura \ref{fig:org2e41118})
\begin{equation}
...
\caption{\label{fig:org2e41118}Ángulo de visión del cielo}
\end{figure}
\end{itemize}

\subsection{Ángulo de incidencia y suciedad}
\label{sec:org690cf16}
...
\end{equation}
Este factor depende el ángulo de incidencia \(theta_s\) y del coeficiente de pérdidas angulares \(a_r\). Cuando la radiación es perpendicular a la superficie (\(\theta_s=0\)), \(FTB\) es cero. En la figura \ref{fig:orgb002613} se puede observar que las pérdidas angulares son más significativas cuando \(\theta_s\) supera los 60º, y se acentúan con mayor suciedad.
\begin{figure}[]
\centering
\includegraphics[keepaspectratio,width=0.8\textwidth,height=0.5\textheight]{figuras/Suciedad.pdf}
\caption{\label{fig:orgb002613}Pérdidas angulares de un módulo fotovoltaico para diferentes grados de suciedad en función del ángulo de incidencia.}
\end{figure}
...
\label{sec:orgd95e0d9}
El punto de máxima potencia es aquel situado en la curva de funcionamiento del generador donde, como su propio nombre indica, los valores de tensión y corriente son tales que la potencia que entrega es máxima (figura \ref{fig:org2c37003}).
\begin{figure}[]
\centering
\includegraphics[keepaspectratio,width=0.8\textwidth,height=0.5\textheight]{figuras/CurvaIV_Ta20_G800.pdf}
\caption{\label{fig:org2c37003}Curvas corriente-tensión(línea discontinua) y potencia-tensión(línea continua) de una célula solar (\(T_a=20^\circ C\) y \(G=800 w/m^2\))}
\end{figure}

\subsubsection{Factor de forma y eficiencia}
\label{sec:org5bc5e23}
El área encerrada por el rectángulo definido por el producto \(I_{mpp}\cdot V_{mpp}\) es, como e observable en la figura \ref{fig:org2c37003}, inferiro a la respresentada por el producto \(I_{sc}\cdot V_{oc}\). La relación entre estad dos superficies se cuantifica con el factor de forma:\nomenclature[MPP]{MPP}{Punto de máxima potencia de un dispositivo fotovoltaico}
\begin{equation}
...
\subsection{Funcionamiento de un generador fotovoltaico}
\label{sec:orgdc078c7}
\label{subsec:funcionamiento-generador-fotovoltaico}
Un generador fotovoltaico es un asociación eléctrica de módulo fotovoltaicos para adaptarse a las condiciones de funcionamiento de una aplicación determinada. Se compone de un total de \(N_p\cdot N_s=N_T\) módulos, siendo \(N_p\) el número de ramas (módulo en paralelo), y \(N_s\) el número de módulo en cada serie. El número de ramas define la corriente total del generador, \(I_{sc,g}=N_p\cdot I_{sc,m}\), y el número de módulos por serie define la tensión del generador, \(V_{oc,g}=N_s\cdot V_{oc,m}\). Por lo tanto, la potencia se puede calcular mediante la ecuación \ref{eq:pot-gen-fv}.
\begin{equation}
P_g=N_T\cdot P_m=(N_s\cdot V_{mpp,m})(N_p\cdot I_{mpp,m})
\label{eq:pot-gen-fv}
\end{equation}

\subsubsection{Pérdidas por dispersión}
\label{sec:org22965bf}
\label{perdidas-dispersion}
Los parámetros eléctricos de los módulos fotovoltaicos que componene un generador nunca son exactamente iguales. La dispersión de estos parámetreos provoca que la potencia eléctrica total sea inferior a la suma de las individuales. Las pérdidas de dispersión por corriente aumentan con el número de módulos que se colocan en serie, mientras que, las pérdidas por tensión aumentan con el número de modulos en paralelo, sin embargo, estas últimas se pueden despreciar.

\subsection{Sistemas Fotovoltaicos de Conexión a Red}
\label{sec:orgf2afc98}
\label{subsec:sistemas-fotovoltaicos-conexion-red}
Un Sistema Fotovoltaico Conectado a la Red (SFCR\nomenclature[SFCR]{\(SFCR\)}{Sistema fotovoltaico de conexión a red}) es un sistema cuya función es producir energía elétrica en condiciones adecuadas para poder inyectada en la red convencional. Como se muestra en la figura \ref{fig:sch-sfcr}, un SFCR se compone del generador fotovoltaico, un inversor DC/AC y un conjunto de protecciones eléctricas.
\begin{figure}[]
\centering
\includegraphics[keepaspectratio,width=0.8\textwidth,height=0.5\textheight]{figuras/EsquemaSFCR.pdf}
\caption{Esquema de un SFCR \label{fig:sch-sfcr}}
\end{figure}

\subsubsection{Orientación e inclinación del generador}
\label{sec:orgd357f09}
En los sistemas fotovoltaicos, la orientación del generador debe ser hacia el horizonte Sur en el hemisferio Norte y hacia el horizonte Norte en el hemisferio Sur. La inclinación óptima del generador para maximizar la producción anual se encuentra entre la inclinación que favorece la producción en invierno y la que maximiza en verano, aunque se pueden obtener valores más precisos con la ecuación \ref{eq:beta-opt}:\nomenclature[betaopt]\{\(\beta_{opt}\)\}\{Ángulo de inclinación que optimiza el funcionamiento de un sistema fotovoltaico\}
\begin{equation}
\beta_{opt}=3,7+0,69\cdot |\phi|
\label{eq:beta-opt}
\end{equation}
Es importante que la inclinación no sea inferior a 15º para que la lluvia pueda limpiar la suciedad acumulada.Sin embargo, las pérdidas por reflexión sólo son importantes a partir de un ángulo en torno a 70º (figura).
\begin{figure}[]
\centering
\includegraphics[keepaspectratio,width=0.8\textwidth,height=0.5\textheight]{figuras/PorcentajeProduccionEdificios.pdf}
\caption{Energía producida por un SFCR con una orientación e inclinacióndeterminada respecto a la energía producida por el mismo SFCR con la orientación e inclinación óptimas.\label{fig:EnergiaorientacionInclinacion}}
\end{figure}

\subsubsection{Modulos en serie}
\label{sec:org5facace}
El inversor está diseñado para soportar una tensión máxima en la entrada. Superarla puede conllevar la avería del equipo. Con la ecuación \ref{eq:ten-max-inv} determinamos el máximo número de módulos en serie(\(N_{sMAX}\)):
\begin{equation}
V_{ocG}(G=200\frac{W}{m^2}, T_a=-10^\circ C)<V_{max, inv}
\label{eq:ten-max-inv}
\end{equation}

\section{Operaciones del paquete \texttt{solaR2}}
\label{sec:org515800b}
\label{sec:operaciones-paquete}
En la figura \ref{fig:org182dc84}, se muestra el proceso de cálculo que sigue el paquete a la hora de obtener la estimación de la producción del sistema fotovoltaico.
\begin{figure}[]
\centering
\includegraphics[keepaspectratio,width=0.8\textwidth,height=0.5\textheight]{figuras/procedure.pdf}
\caption{\label{fig:org182dc84}Proceso de cálculo de las funciones de \texttt{solaR2}}
\end{figure}
